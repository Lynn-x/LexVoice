...
- **会议概览图**：基于当前可用的全场转写生成 Mermaid 概览，可单独更新并随 HTML、PDF 导出；图失败时保留文字，超长报告 PDF 使用分页。
- **纪要恢复**：已有可用会中转写时可先交付纪要，明确标注来源和覆盖情况；支持利用成功片段、重新关联移动后的原笔记，以及按当前设置创建独立新任务。
- **人物归属修订**：批量归属和逐条纠正支持撤销，不回滚其他正文修改；不新增跨段声纹自动识别。
- **正文独立交付**：结束录音不再等待大纲和问答积压补跑；重新整理另存并打开新版本，不覆盖已有结果。
- **录音与服务**：恢复非流式录音提前切段，防止同名笔记冲突，改善星辰本机内存不足时的处理及百炼上传兼容性，区分按量付费与 Token Plan。
- **侧栏与材料**：纪要默认按时间显示全部，修复录音中议程提交、处理进度和提示框定位问题。

整理纪要的自动概览默认开启，可在设置中关闭。生成概览可能增加 AI 接口费用；仅渲染已有概览不会再次请求 AI。更新本身不会批量改写旧笔记。

详情见 [2.6.0 版本说明](RELEASE_NOTES.md)。
...
详情见 [2.5.0 版本说明](https://github.com/Lynn-x/LexVoice/releases/tag/2.5.0)。
...
录音时，章节会随着会议内容逐步生成，不必等到录音结束才知道刚才讨论了什么。录音结束后，大纲章节会与播放器联动；点击章节即可跳转到对应的录音位置。停止录音后，根据已保存的转写整理纪要，未完成的大纲更新不再阻断正文交付。
