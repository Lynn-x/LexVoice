...
- **会前材料**：录音前创建会议笔记，粘贴文字或导入图片、PDF；与会人、主题和议程写回同一篇 Markdown。
- **关注事项回应**：对照会前问题和明确列出的议程，保留转写依据；没有依据的内容标为待核对，正文继续覆盖实际讨论。
- **独立说话人页**：从会前名单选择姓名，集中核对编号和发言归属。
- **复制与拖拽**：自由选中实时大纲；桌面端从纪要列表拖出文件或路径。
- **服务兼容**：修复部分 M4A 说话人转写失败、流式连接测试和百炼模型列表读取问题。

数据、费用和兼容性边界见 [2.3.0 版本说明](RELEASE_NOTES.md)。
...
开启会后说话人分离后，LexVoice 会先使用临时编号完成转写，不等待人工填写姓名。转写完成后，在独立的“说话人”页中从会前名单选择姓名，也可手工命名、把多个编号归到同一人名下，或修正某一句的归属。系统不会只凭会议文字猜测真实身份；未确认的内容会继续保留为编号。
